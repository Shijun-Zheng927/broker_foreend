    ...
    <template v-slot:form>
      <el-form label-position="left" label-width="150px" :model="formData">
        ...
        <el-form-item label="分片管理Id">
          <el-input v-model="formData.uploadId" placeholder="分片上传获取到的id"></el-input>
        </el-form-item>
        <el-form-item label="每页最大分片数">
          <el-input v-model="formData.maxParts" placeholder="10"></el-input>
        </el-form-item>
        <el-form-item label="分片起始位置">
          <el-input v-model="formData.marker" placeholder="1"></el-input>
        </el-form-item>
      </el-form>
    </template>
...
export default {
  ...
  methods:{
    send(){
      this.axios.post('/listPartsByPaper',this.formData).then((res)=>{
